...
def defThreshold(data,ratio):
    value = list(data.values())
    value.sort(reverse=True)
    index = int(len(value) * ratio)
    return value[index]
...
def waveCluster(data,scale=50,wavelet='db2',threshold=0.5,plot=False):
    ...
    if threshold<0:
        ratio = (threshold*-1)/100
        threshold = defThreshold(dwtResult,ratio)
    ...
    answer = np.zeros_like(tags)
    tags_i = np.unique(tags)
    j = 0
    for i in tags_i:
        index = np.argwhere(tags==i).reshape([-1])
        answer[index] = j
        j+=1
    return(answer)
